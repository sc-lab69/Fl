...
class MiniWasteGame {
  ...
  initCanvas() {
    ...
    const width = Math.min(parent.clientWidth - 32, 900);
    const height = 500;
    ...
    const binY = height - binHeight - 20;
  ...
  handlePointerUp() {
    ...
    for (const bin of this.bins) {
      if (
        item.x >= bin.x - 10 &&
        item.x <= bin.x + bin.w + 10 &&
        item.y >= bin.y - 45 &&
        item.y <= bin.y + bin.h + 10
      ) {
        droppedInBin = bin;
        break;
      }
  ...
  drawBackground() {
    ...
    // Soft ground plane contact shadow
    const groundGrad = ctx.createLinearGradient(0, canvas.height - 35, 0, canvas.height);
    ...
    ctx.fillRect(0, canvas.height - 31, canvas.width, 1.5);
  }
  ...
  draw3DBins() {
    ...
    for (const bin of this.bins) {
      ...
      ctx.beginPath();
      ctx.ellipse(cx, y + h + 10, w * 0.48, 8, 0, 0, Math.PI * 2);
      ctx.fill();
      ...
      const logoY = bodyTopY + (bodyBotY - bodyTopY) * 0.44;
      this.draw3DRecycleLogo(cx, logoY, w * 0.19);
      ...
      // 8. Bin Title Badge Below
      ctx.fillStyle = '#0f172a';
      ctx.font = 'bold 13px Prompt, sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText(bin.title, cx, y + h + 24);

      ctx.fillStyle = '#64748b';
      ctx.font = '10px Prompt, sans-serif';
      ctx.fillText(bin.sub, cx, y + h + 37);
